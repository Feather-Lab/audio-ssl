...
class LitAudioSSL(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        step_type = 'train'
        ssl_opt, class_opt = self.optimizers()
        ssl_opt.zero_grad()
        class_opt.zero_grad()
        ...
        #####################################
        # Optimize ResNet for SSL objective #
        #####################################
        self.toggle_optimizer(ssl_opt)
        ssl_opt.step()
        self.untoggle_optimizer(ssl_opt)
        ...
        ##############################  
        # Step LR scheduler for LARS #
        ##############################  
        if self.config['hparas'].get('lr_schedule', True):
            sch = self.lr_schedulers()
            sch.step()
        return None # No returns anymore in manual opt. 
    ...
    def configure_optimizers(self):
        # get params for classifier and backbone separately
        classifier_params = self.model.model.lin_cls.parameters()
        ssl_params = self.model.model.f.parameters()
        # Word classifier optimizer 
        word_optimizer = torch.optim.AdamW(classifier_params, lr=1e-5)
        # get bool for lr scheudle - default is true 
        use_lr_schedule = self.config['hparas'].get('lr_schedule', True)
        # SSL Optimizer
        if self.config['hparas']['optimizer'] == "LARS":
            if use_lr_schedule:
                ...
                lr_scheduler = CosineWarmupScheduler(
                    ...
                    lr=self.config['hparas']['lr']                         # is base lr for schedule 
                )
                return [ssl_optimizer, word_optimizer], [
                    {
            ...
            ssl_optimizer = opt(ssl_params, lr=lr)      
        return [ssl_optimizer, word_optimizer]
    ...
    def total_training_steps(self) -> int:
        dataset_size = len(self.train_dataloader()) # is pre-batched 
