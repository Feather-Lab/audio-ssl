"""
ESC-50 SVM linear probe evaluation for a pretrained Whisper encoder.

Mirrors eval_audiomae_esc50.py but uses the Whisper encoder
(whisper_encoder_arch.WhisperLayerwiseEncoder) with layerwise
activation extraction.  For each specified layer, extracts time-pooled
features for every ESC-50 clip, runs 5-fold cross-validated LinearSVC,
...
def extract_features(
    ...
    """Extract Whisper features for a list of audio files.
    ...
    cache_dir = Path(scratch_dir) / f"whisper_{encoder.whisper_model_name}_activations" / layer
    ...
    for idx, audio_path in enumerate(audio_paths):
        ...
        if cache_file.exists() and not overwrite:
            ...
            for _ in range(num_reps):
                ...
                waveform = torch.from_numpy(sound).float().unsqueeze(0).to(device)  # (1, T)

                embeddings = encoder(waveform, sr=WHISPER_SR, flatten_activations=False)
                act = embeddings[layer]  # (1, n_ctx, n_state)
                pooled = act.mean(dim=1).cpu().numpy().ravel()  # time-pool → (n_state,)
...
# ---------------------------------------------------------------------------
# SVM training (reused from eval_audiomae_esc50.py)
# ---------------------------------------------------------------------------
...
def run_esc50_eval(args):
    ...
    encoder = WhisperLayerwiseEncoder(whisper_model_name=args.whisper_model).to(device)
    ...
    print(f"Evaluating layer: {layer}  (total layers: {len(layer_names)})")

    net_name = f"whisper_{args.whisper_model}"
    folds = [1, 2, 3, 4, 5]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Whisper ESC-50 SVM evaluation")
    parser.add_argument("-L", "--layer_idx", type=int, default=None,
                        help="Layer index (0..N-1 for blocks, N for ln_post). "
                             "Overrides --layer_name.")
    parser.add_argument("--layer_name", type=str, default="ln_post",
                        help="Layer name (e.g. encoder_block_5, ln_post). Used if --layer_idx not given.")
    parser.add_argument("--whisper_model", type=str, default="large-v3",
                        help="Whisper model name (e.g. tiny, base, large-v3, large-v3-turbo).")
    ...
    parser.add_argument("-A", "--num_activations", type=int, default=4096,
                        help="Kept for naming compatibility.")
