...
def main(args):
    ...
    regression_1_db_snr = LinearRegression().fit(X1_train, Y_train_db_SNR)
    regression_2_db_snr = LinearRegression().fit(X2_train, Y_train_db_SNR)
    ...
    regression_1_filter = LinearRegression().fit(X1_train_filter, Y_train_filter)
    regression_2_filter = LinearRegression().fit(X2_train_filter, Y_train_filter)
    ...
    print(f"{score_1_db_snr:.3f} (dB SNR), {score_2_db_snr:.3f} (dB SNR)")
    print(f"{score_1_filter:.3f} (filter), {score_2_filter:.3f} (filter)")
    ...
    fig_out_dir = Path('parameter_decoding') / f"{invar_model_name}_{equivar_model_name}"
    fig_out_dir.mkdir(parents=True, exist_ok=True)
    fig_out_name = fig_out_dir / f"{layer}_param_decoding_pearsons_r2_by_augmentation"
    plt.savefig(fig_out_name, transparent=False, bbox_inches='tight' )
