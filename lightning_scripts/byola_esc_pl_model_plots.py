...
if __name__ == '__main__':
    ...
    model = BYOLAModule(config=config).eval().cuda()

    DUR_SECS=2
  

    # model, ds, metamer_layers = build_network.main(return_metamer_layers=True)
    layer = 'final' ## TODO: softcode layer at later time 
    print("Layer name: "+layer)
    # net_path = os.getcwd()
    # net_name = '-'.join(net_path.split('/')[-2:])
    net_name = "byola" # model name is stem of yaml config 
