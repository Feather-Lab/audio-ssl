import json
import math
import random
from collections import defaultdict
from pathlib import Path
from typing import Dict, List, Tuple

import torch
import torch.nn.functional as F
import torchaudio

from default_paths import NSYNTH_DIR


class NsynthTripletDataset(torch.utils.data.Dataset):
    """Self-contained NSynth interval-triplet generator with resampling."""

    def __init__(
        self,
        nsynth_root: Path | None = None,
        split: str = "valid",
        target_sr: int = 20_000,
        n_examples: int = 300,
        seed: int = 0,
        note_duration: float = 0.5,
        lead_in: float = 0.25,
        mid_gap: float = 0.4,
        total_clip: float = 2.0,
        fade_ms: float = 50.0,
        min_interval: int = 1,
        max_interval: int = 12,
        min_midi: int = 40,
        max_midi: int = 72,
        experiment_type: str = "interval_match",
        balance_eval: bool = True,
        balance_by_family: bool = True,
    ):
        super().__init__()
        if nsynth_root is None:
            if NSYNTH_DIR is None:
                raise RuntimeError("Set COCHDNN_NSYNTH_DIR to the local NSynth directory.")
            nsynth_root = NSYNTH_DIR
        self.nsynth_root = Path(nsynth_root)
        self.split = split
        self.target_sr = target_sr
        self.requested_n_examples = n_examples
        self.n_examples = n_examples
        self.seed = seed
        self.note_duration = note_duration
        self.lead_in = lead_in
        self.mid_gap = mid_gap
        self.total_clip = total_clip
        self.fade_ms = fade_ms
        self.min_interval = min_interval
        self.max_interval = max_interval
        self.min_midi = min_midi
        self.max_midi = max_midi
        self.balance_eval = balance_eval
        self.balance_by_family = balance_by_family

        if experiment_type not in [
            "interval_match",
            "direction_match",
            "direction_transpose",
            "timbre_order_match",
            "melody_match",
            "instrument_match",
        ]:
            raise ValueError(
                "experiment_type must be 'interval_match', 'direction_match', "
                "'direction_transpose', 'timbre_order_match', 'melody_match', or "
                f"'instrument_match', got {experiment_type}"
            )
        
        # Map melody_match to timbre_order_match for backward compatibility
        if experiment_type == "melody_match":
            experiment_type = "timbre_order_match"
        
        self.experiment_type = experiment_type

        self.metadata_path = self.nsynth_root / f"nsynth-{split}" / "examples.json"
        self.audio_dir = self.nsynth_root / f"nsynth-{split}" / "audio"
        if not self.metadata_path.exists():
            raise FileNotFoundError(f"Missing metadata: {self.metadata_path}")
        if not self.audio_dir.exists():
            raise FileNotFoundError(f"Missing audio dir: {self.audio_dir}")

        with self.metadata_path.open() as f:
            self.metadata: Dict[str, Dict] = json.load(f)

        self.pitch_instrument_to_ids: Dict[int, Dict[str, List[str]]] = defaultdict(lambda: defaultdict(list))
        self.instruments = set()
        self.instrument_to_family: Dict[str, str] = {}
        self.family_to_instruments: Dict[str, set[str]] = defaultdict(set)
        for file_id, meta in self.metadata.items():
            pitch = int(meta["pitch"])
            instrument = meta["instrument_str"]
            family = meta["instrument_family_str"]
            self.pitch_instrument_to_ids[pitch][instrument].append(file_id)
            self.instruments.add(instrument)
            if instrument in self.instrument_to_family and self.instrument_to_family[instrument] != family:
                raise ValueError(f"Instrument {instrument} has inconsistent family labels")
            self.instrument_to_family[instrument] = family
            self.family_to_instruments[family].add(instrument)
        self.instruments = sorted(self.instruments)
        self.instrument_families = sorted(self.family_to_instruments)
        self.family_to_instruments = {
            family: sorted(instruments) for family, instruments in self.family_to_instruments.items()
        }
        self.available_pitches = sorted(self.pitch_instrument_to_ids)

        pos = list(range(self.min_interval, self.max_interval + 1))
        neg = [-i for i in pos]
        self.interval_choices = pos + neg

        if self.balance_eval:
            self.conditions = self._build_balanced_conditions()
            self.n_conditions = len(self.conditions)
            if self.n_conditions == 0:
                raise RuntimeError("No valid interval/instrument conditions available for balanced eval")
            if self.requested_n_examples < self.n_conditions:
                self.n_examples = self.n_conditions
            else:
                self.n_examples = (self.requested_n_examples // self.n_conditions) * self.n_conditions
            repeats = self.n_examples // self.n_conditions
            self.condition_schedule = self.conditions * repeats
            schedule_rng = random.Random(self.seed)
            schedule_rng.shuffle(self.condition_schedule)

    # ---------- helpers ----------
    def _load_audio(self, file_id: str) -> Tuple[torch.Tensor, int]:
        audio_path = self.audio_dir / f"{file_id}.wav"
        waveform, sr = torchaudio.load(str(audio_path))
        if waveform.shape[0] > 1:
            waveform = waveform[0]
        return waveform.squeeze(0), sr

    def _crop_with_cosine_fade(self, audio: torch.Tensor, sr: int) -> torch.Tensor:
        target_samples = int(sr * self.note_duration)
        if audio.numel() >= target_samples:
            audio = audio[:target_samples]
        else:
            audio = F.pad(audio, (0, target_samples - audio.numel()))
        ramp_samples = int(sr * self.fade_ms / 1000.0)
        if ramp_samples > 0 and target_samples >= ramp_samples:
            t = torch.linspace(0, math.pi, ramp_samples, device=audio.device)
            fade = 0.5 * (1 + torch.cos(t))
            audio[-ramp_samples:] = audio[-ramp_samples:] * fade
        return audio

    def _valid_interval_starts(self, interval: int, instrument: str) -> List[int]:
        starts: List[int] = []
        for p in self.available_pitches:
            if not (self.min_midi <= p <= self.max_midi):
                continue
            target = p + interval
            if not (self.min_midi <= target <= self.max_midi):
                continue
            if instrument in self.pitch_instrument_to_ids.get(p, {}) and instrument in self.pitch_instrument_to_ids.get(target, {}):
                starts.append(p)
        return starts

    def _choose_file(self, pitch: int, instrument: str, rng: random.Random) -> str:
        return rng.choice(self.pitch_instrument_to_ids[pitch][instrument])

    def _instrument_has_pitch(self, pitch: int, instrument: str) -> bool:
        return instrument in self.pitch_instrument_to_ids.get(pitch, {})

    def _has_interval_match_config(self, interval: int, instrument: str) -> bool:
        anchor_interval_size = abs(interval)
        candidate_negative_intervals = [
            delta for delta in self.interval_choices if abs(delta) != anchor_interval_size
        ]
        starts = self._valid_interval_starts(interval, instrument)
        if len(starts) < 2 or not candidate_negative_intervals:
            return False
        for anchor_start in starts:
            for transposed_start in starts:
                if transposed_start == anchor_start:
                    continue
                for delta in candidate_negative_intervals:
                    target_pitch = transposed_start + delta
                    if not (self.min_midi <= target_pitch <= self.max_midi):
                        continue
                    if self._instrument_has_pitch(target_pitch, instrument):
                        return True
        return False

    def _has_direction_match_config(self, interval: int, instrument: str) -> bool:
        sign = 1 if interval > 0 else -1
        anchor_starts = self._valid_interval_starts(interval, instrument)
        if not anchor_starts:
            return False
        alt_sizes = [i for i in range(self.min_interval, self.max_interval + 1) if i != abs(interval)]
        for size in alt_sizes:
            alt_interval = sign * size
            pos_starts = set(self._valid_interval_starts(alt_interval, instrument))
            neg_starts = set(self._valid_interval_starts(-alt_interval, instrument))
            for anchor_start in anchor_starts:
                if anchor_start in pos_starts and anchor_start in neg_starts:
                    return True
        return False

    def _has_timbre_order_match_config(self, interval: int, instrument: str) -> bool:
        starts = self._valid_interval_starts(interval, instrument)
        if not starts:
            return False
        for anchor_start in starts:
            anchor_target = anchor_start + interval
            for other in self.instruments:
                if other == instrument:
                    continue
                if self._instrument_has_pitch(anchor_start, other) and self._instrument_has_pitch(anchor_target, other):
                    return True
        return False

    def _has_direction_transpose_config(self, interval: int, instrument: str) -> bool:
        starts = self._valid_interval_starts(interval, instrument)
        if len(starts) < 2:
            return False
        for anchor_start in starts:
            anchor_target = anchor_start + interval
            for transpose_start in starts:
                if transpose_start == anchor_start:
                    continue
                shift = transpose_start - anchor_start
                transpose_target = anchor_target + shift
                if self._instrument_has_pitch(transpose_target, instrument):
                    return True
        return False

    def _has_instrument_match_config(self, interval: int, anchor_family: str | None = None) -> bool:
        families = [anchor_family] if anchor_family is not None else self.instrument_families
        for family in families:
            family_instruments = set(self.family_to_instruments[family])
            if len(family_instruments) < 2:
                continue
            for start_pitch in self.available_pitches:
                if not (self.min_midi <= start_pitch <= self.max_midi):
                    continue
                target_pitch = start_pitch + interval
                if not (self.min_midi <= target_pitch <= self.max_midi):
                    continue
                instruments_with_both = (
                    set(self.pitch_instrument_to_ids.get(start_pitch, {}))
                    & set(self.pitch_instrument_to_ids.get(target_pitch, {}))
                )
                anchor_candidates = instruments_with_both & family_instruments
                if len(anchor_candidates) < 2:
                    continue
                negative_candidates = instruments_with_both - family_instruments
                if negative_candidates:
                    return True
        return False

    def _valid_groups_for_interval(self, interval: int) -> List[str]:
        valid: List[str] = []
        if self.balance_by_family:
            for family in self.instrument_families:
                if self.experiment_type == "instrument_match":
                    if self._has_instrument_match_config(interval, anchor_family=family):
                        valid.append(family)
                    continue
                for inst in self.family_to_instruments[family]:
                    if self.experiment_type == "interval_match":
                        if self._has_interval_match_config(interval, inst):
                            valid.append(family)
                            break
                    elif self.experiment_type == "direction_match":
                        if self._has_direction_match_config(interval, inst):
                            valid.append(family)
                            break
                    elif self.experiment_type == "direction_transpose":
                        if self._has_direction_transpose_config(interval, inst):
                            valid.append(family)
                            break
                    elif self.experiment_type == "timbre_order_match":
                        if self._has_timbre_order_match_config(interval, inst):
                            valid.append(family)
                            break
        else:
            for inst in self.instruments:
                if self.experiment_type == "interval_match":
                    if self._has_interval_match_config(interval, inst):
                        valid.append(inst)
                elif self.experiment_type == "direction_match":
                    if self._has_direction_match_config(interval, inst):
                        valid.append(inst)
                elif self.experiment_type == "direction_transpose":
                    if self._has_direction_transpose_config(interval, inst):
                        valid.append(inst)
                elif self.experiment_type == "timbre_order_match":
                    if self._has_timbre_order_match_config(interval, inst):
                        valid.append(inst)
                elif self.experiment_type == "instrument_match":
                    if self._has_instrument_match_config(interval):
                        valid.append(inst)
        return valid

    def _build_balanced_conditions(self) -> List[Tuple[int, str]]:
        conditions: List[Tuple[int, str]] = []
        for interval in self.interval_choices:
            for inst in self._valid_groups_for_interval(interval):
                conditions.append((interval, inst))
        return conditions

    def _build_triplet_for_condition(self, interval: int, instrument: str, rng: random.Random) -> Dict[str, Dict]:
        if self.balance_by_family:
            family = instrument
            if self.experiment_type == "instrument_match":
                return self._build_instrument_match_triplet(interval, rng, anchor_family=family)
            instrument = rng.choice(self.family_to_instruments[family])
        if self.experiment_type == "interval_match":
            return self._build_interval_match_triplet(interval, rng, instrument=instrument)
        if self.experiment_type == "direction_match":
            return self._build_direction_match_triplet(interval, rng, instrument=instrument)
        if self.experiment_type == "direction_transpose":
            return self._build_direction_transpose_triplet(interval, rng, instrument=instrument)
        if self.experiment_type == "timbre_order_match":
            return self._build_timbre_order_match_triplet(interval, rng, anchor_instrument=instrument)
        if self.experiment_type == "instrument_match":
            return self._build_instrument_match_triplet(interval, rng)
        raise ValueError(f"Unknown experiment_type: {self.experiment_type}")

    def _build_triplet(self, interval: int, rng: random.Random) -> Dict[str, Dict]:
        if interval == 0 or abs(interval) < self.min_interval or abs(interval) > self.max_interval:
            raise ValueError(f"Interval must be in ±[{self.min_interval},{self.max_interval}] and non-zero")

        if self.experiment_type == "interval_match":
            return self._build_interval_match_triplet(interval, rng)
        elif self.experiment_type == "direction_match":
            return self._build_direction_match_triplet(interval, rng)
        elif self.experiment_type == "direction_transpose":
            return self._build_direction_transpose_triplet(interval, rng)
        elif self.experiment_type == "timbre_order_match":
            return self._build_timbre_order_match_triplet(interval, rng)
        elif self.experiment_type == "instrument_match":
            return self._build_instrument_match_triplet(interval, rng)
        else:
            raise ValueError(f"Unknown experiment_type: {self.experiment_type}")

    def _build_interval_match_triplet(
        self,
        interval: int,
        rng: random.Random,
        instrument: str | None = None,
    ) -> Dict[str, Dict]:
        """Interval match: same instrument; positive is transposed with same interval; negative uses a different interval size."""
        anchor_interval_size = abs(interval)
        candidate_negative_intervals = [
            delta for delta in self.interval_choices if abs(delta) != anchor_interval_size
        ]
        if not candidate_negative_intervals:
            raise RuntimeError("No negative interval choices available")

        candidate_configs = []
        instruments = [instrument] if instrument is not None else self.instruments
        for inst in instruments:
            starts = self._valid_interval_starts(interval, inst)
            if len(starts) < 2:
                continue
            for anchor_start in starts:
                for transposed_start in starts:
                    if transposed_start == anchor_start:
                        continue
                    neg_intervals = []
                    for delta in candidate_negative_intervals:
                        target_pitch = transposed_start + delta
                        if not (self.min_midi <= target_pitch <= self.max_midi):
                            continue
                        if inst in self.pitch_instrument_to_ids.get(target_pitch, {}):
                            neg_intervals.append(delta)
                    if neg_intervals:
                        candidate_configs.append((inst, anchor_start, transposed_start, neg_intervals))

        if not candidate_configs:
            raise RuntimeError(f"No instruments support interval {interval} with valid negative intervals")

        instrument, anchor_start, transposed_start, neg_intervals = rng.choice(candidate_configs)
        negative_interval = rng.choice(neg_intervals)

        anchor_target = anchor_start + interval
        positive_start = transposed_start
        positive_target = positive_start + interval
        negative_start = transposed_start
        negative_target = negative_start + negative_interval

        positive_start_id = self._choose_file(positive_start, instrument, rng)

        triplet = {
            "anchor": {
                "instrument": instrument,
                "start_pitch": anchor_start,
                "target_pitch": anchor_target,
                "interval": interval,
                "interval_size": anchor_interval_size,
                "start_id": self._choose_file(anchor_start, instrument, rng),
                "target_id": self._choose_file(anchor_target, instrument, rng),
            },
            "positive": {
                "instrument": instrument,
                "start_pitch": positive_start,
                "target_pitch": positive_target,
                "interval": interval,
                "interval_size": anchor_interval_size,
                "start_id": positive_start_id,
                "target_id": self._choose_file(positive_target, instrument, rng),
            },
            "negative": {
                "instrument": instrument,
                "start_pitch": negative_start,
                "target_pitch": negative_target,
                "interval": negative_interval,
                "interval_size": abs(negative_interval),
                "start_id": positive_start_id,
                "target_id": self._choose_file(negative_target, instrument, rng),
            },
        }
        return triplet

    def _build_direction_match_triplet(
        self,
        interval: int,
        rng: random.Random,
        instrument: str | None = None,
    ) -> Dict[str, Dict]:
        """Direction match: same instrument for all examples.
        Positive/negative share the same (different) interval size from the anchor.
        Positive matches anchor direction; negative reverses it."""
        if instrument is None:
            instrument = rng.choice(self.instruments)

        sign = 1 if interval > 0 else -1
        anchor_starts = self._valid_interval_starts(interval, instrument)
        if not anchor_starts:
            raise RuntimeError(f"No {instrument} clips support anchor interval {interval}")
        anchor_start = rng.choice(anchor_starts)
        anchor_target = anchor_start + interval

        alt_sizes = [i for i in range(self.min_interval, self.max_interval + 1) if i != abs(interval)]
        rng.shuffle(alt_sizes)
        for size in alt_sizes:
            alt_interval = sign * size
            positive_starts = set(self._valid_interval_starts(alt_interval, instrument))
            negative_starts = set(self._valid_interval_starts(-alt_interval, instrument))
            if anchor_start not in positive_starts or anchor_start not in negative_starts:
                continue
            positive_start = anchor_start
            positive_target = anchor_start + alt_interval
            negative_start = anchor_start
            negative_target = anchor_start - alt_interval
            triplet = {
                "anchor": {
                    "instrument": instrument,
                    "start_pitch": anchor_start,
                    "target_pitch": anchor_target,
                    "interval": interval,
                    "start_id": self._choose_file(anchor_start, instrument, rng),
                    "target_id": self._choose_file(anchor_target, instrument, rng),
                },
                "positive": {
                    "instrument": instrument,
                    "start_pitch": positive_start,
                    "target_pitch": positive_target,
                    "interval": alt_interval,
                    "start_id": self._choose_file(positive_start, instrument, rng),
                    "target_id": self._choose_file(positive_target, instrument, rng),
                },
                "negative": {
                    "instrument": instrument,
                    "start_pitch": negative_start,
                    "target_pitch": negative_target,
                    "interval": -alt_interval,
                    "start_id": self._choose_file(negative_start, instrument, rng),
                    "target_id": self._choose_file(negative_target, instrument, rng),
                },
            }
            return triplet
        raise RuntimeError(f"No valid direction-match config for {instrument} with anchor interval {interval}")

    def _build_timbre_order_match_triplet(
        self,
        interval: int,
        rng: random.Random,
        anchor_instrument: str | None = None,
    ) -> Dict[str, Dict]:
        """Anchor and positive share MIDI values but different instruments.
        Negative uses same instrument as positive but swaps the order of MIDI notes."""
        # Find instruments that support the interval
        instrument_to_starts: Dict[str, List[int]] = {}
        instruments = [anchor_instrument] if anchor_instrument is not None else self.instruments
        for inst in instruments:
            starts = self._valid_interval_starts(interval, inst)
            if len(starts) >= 1:
                instrument_to_starts[inst] = starts
        if anchor_instrument is not None:
            if anchor_instrument not in instrument_to_starts:
                raise RuntimeError(f"Anchor instrument {anchor_instrument} does not support interval {interval}")
        elif len(instrument_to_starts) < 2:
            raise RuntimeError(f"Need at least 2 instruments supporting interval {interval}, found {len(instrument_to_starts)}")

        # Choose anchor instrument and start pitch
        anchor_instrument = anchor_instrument or rng.choice(list(instrument_to_starts.keys()))
        anchor_starts = instrument_to_starts[anchor_instrument]
        anchor_start = rng.choice(anchor_starts)
        anchor_target = anchor_start + interval

        # Find a different instrument that has both anchor_start and anchor_target pitches
        candidate_positive_instruments = []
        for inst in self.instruments:
            if inst == anchor_instrument:
                continue
            if (inst in self.pitch_instrument_to_ids.get(anchor_start, {}) and 
                inst in self.pitch_instrument_to_ids.get(anchor_target, {})):
                candidate_positive_instruments.append(inst)
        
        if not candidate_positive_instruments:
            raise RuntimeError(f"No other instrument has both pitches {anchor_start} and {anchor_target}")

        positive_instrument = rng.choice(candidate_positive_instruments)
        
        # Positive: same MIDI values as anchor, different instrument
        positive_start = anchor_start
        positive_target = anchor_target

        # Negative: same instrument as positive, but swapped order (target → start)
        negative_start = anchor_target  # This was the target in anchor/positive
        negative_target = anchor_start  # This was the start in anchor/positive
        negative_interval = -interval  # Swapped direction

        triplet = {
            "anchor": {
                "instrument": anchor_instrument,
                "start_pitch": anchor_start,
                "target_pitch": anchor_target,
                "interval": interval,
                "start_id": self._choose_file(anchor_start, anchor_instrument, rng),
                "target_id": self._choose_file(anchor_target, anchor_instrument, rng),
            },
            "positive": {
                "instrument": positive_instrument,
                "start_pitch": positive_start,
                "target_pitch": positive_target,
                "interval": interval,
                "start_id": self._choose_file(positive_start, positive_instrument, rng),
                "target_id": self._choose_file(positive_target, positive_instrument, rng),
            },
            "negative": {
                "instrument": positive_instrument,
                "start_pitch": negative_start,
                "target_pitch": negative_target,
                "interval": negative_interval,
                "start_id": self._choose_file(negative_start, positive_instrument, rng),
                "target_id": self._choose_file(negative_target, positive_instrument, rng),
            },
        }
        return triplet

    def _build_direction_transpose_triplet(
        self,
        interval: int,
        rng: random.Random,
        instrument: str | None = None,
    ) -> Dict[str, Dict]:
        """Direction match with transposition: same instrument for all examples.
        Positive is transposed by a fixed shift with same interval direction as anchor.
        Negative is transposed by the same shift but reverses the interval direction."""
        candidate_configs: List[Tuple[str, int, int]] = []
        instruments = [instrument] if instrument is not None else self.instruments
        for inst in instruments:
            starts = self._valid_interval_starts(interval, inst)
            if not starts:
                continue
            for anchor_start in starts:
                anchor_target = anchor_start + interval
                for transpose_start in starts:
                    if transpose_start == anchor_start:
                        continue
                    shift = transpose_start - anchor_start
                    transpose_target = anchor_target + shift
                    if inst in self.pitch_instrument_to_ids.get(transpose_target, {}):
                        candidate_configs.append((inst, anchor_start, shift))

        if not candidate_configs:
            raise RuntimeError(f"No instruments support transposed direction match for interval {interval}")

        instrument, anchor_start, shift = rng.choice(candidate_configs)
        anchor_target = anchor_start + interval
        positive_start = anchor_start + shift
        positive_target = anchor_target + shift

        negative_start = positive_target
        negative_target = positive_start
        negative_interval = -interval

        triplet = {
            "anchor": {
                "instrument": instrument,
                "start_pitch": anchor_start,
                "target_pitch": anchor_target,
                "interval": interval,
                "start_id": self._choose_file(anchor_start, instrument, rng),
                "target_id": self._choose_file(anchor_target, instrument, rng),
            },
            "positive": {
                "instrument": instrument,
                "start_pitch": positive_start,
                "target_pitch": positive_target,
                "interval": interval,
                "start_id": self._choose_file(positive_start, instrument, rng),
                "target_id": self._choose_file(positive_target, instrument, rng),
            },
            "negative": {
                "instrument": instrument,
                "start_pitch": negative_start,
                "target_pitch": negative_target,
                "interval": negative_interval,
                "start_id": self._choose_file(negative_start, instrument, rng),
                "target_id": self._choose_file(negative_target, instrument, rng),
            },
        }
        return triplet

    def _build_instrument_match_triplet(
        self,
        interval: int,
        rng: random.Random,
        anchor_family: str | None = None,
    ) -> Dict[str, Dict]:
        """Instrument match: same MIDI notes, different instruments.
        Anchor/positive are from the same family; negative is from a different family."""
        families = [anchor_family] if anchor_family is not None else self.instrument_families
        candidate_configs: List[Tuple[str, int, List[str], List[str]]] = []
        for family in families:
            family_instruments = set(self.family_to_instruments[family])
            if len(family_instruments) < 2:
                continue
            for start_pitch in self.available_pitches:
                if not (self.min_midi <= start_pitch <= self.max_midi):
                    continue
                target_pitch = start_pitch + interval
                if not (self.min_midi <= target_pitch <= self.max_midi):
                    continue
                instruments_with_both = (
                    set(self.pitch_instrument_to_ids.get(start_pitch, {}))
                    & set(self.pitch_instrument_to_ids.get(target_pitch, {}))
                )
                anchor_candidates = sorted(instruments_with_both & family_instruments)
                if len(anchor_candidates) < 2:
                    continue
                negative_candidates = sorted(inst for inst in instruments_with_both if inst not in family_instruments)
                if not negative_candidates:
                    continue
                candidate_configs.append((family, start_pitch, anchor_candidates, negative_candidates))

        if not candidate_configs:
            raise RuntimeError(f"No instrument-match configs for interval {interval}")

        family, start_pitch, anchor_candidates, negative_candidates = rng.choice(candidate_configs)
        anchor_instrument, positive_instrument = rng.sample(anchor_candidates, 2)
        negative_instrument = rng.choice(negative_candidates)
        target_pitch = start_pitch + interval

        triplet = {
            "anchor": {
                "instrument": anchor_instrument,
                "start_pitch": start_pitch,
                "target_pitch": target_pitch,
                "interval": interval,
                "start_id": self._choose_file(start_pitch, anchor_instrument, rng),
                "target_id": self._choose_file(target_pitch, anchor_instrument, rng),
            },
            "positive": {
                "instrument": positive_instrument,
                "start_pitch": start_pitch,
                "target_pitch": target_pitch,
                "interval": interval,
                "start_id": self._choose_file(start_pitch, positive_instrument, rng),
                "target_id": self._choose_file(target_pitch, positive_instrument, rng),
            },
            "negative": {
                "instrument": negative_instrument,
                "start_pitch": start_pitch,
                "target_pitch": target_pitch,
                "interval": interval,
                "start_id": self._choose_file(start_pitch, negative_instrument, rng),
                "target_id": self._choose_file(target_pitch, negative_instrument, rng),
            },
        }
        return triplet

    def _make_interval_clip(self, start_id: str, target_id: str) -> Tuple[torch.Tensor, int]:
        start_audio, sr_start = self._load_audio(start_id)
        target_audio, sr_target = self._load_audio(target_id)
        if sr_start != sr_target:
            target_audio = torchaudio.functional.resample(target_audio.unsqueeze(0), sr_target, sr_start).squeeze(0)
        sr = sr_start
        start_audio = self._crop_with_cosine_fade(start_audio, sr)
        target_audio = self._crop_with_cosine_fade(target_audio, sr)
        lead_in = torch.zeros(int(sr * self.lead_in))
        gap = torch.zeros(int(sr * self.mid_gap))
        tail = max(0, int(sr * (self.total_clip - (self.lead_in + 2 * self.note_duration + self.mid_gap))))
        tail_pad = torch.zeros(tail)
        clip = torch.cat([lead_in, start_audio, gap, target_audio, tail_pad])
        target_total = int(sr * self.total_clip)
        if clip.numel() > target_total:
            clip = clip[:target_total]
        elif clip.numel() < target_total:
            clip = torch.cat([clip, torch.zeros(target_total - clip.numel())])
        return clip, sr

    def _load_triplet_audio(self, triplet: Dict[str, Dict]) -> Tuple[Dict[str, torch.Tensor], int]:
        clips: Dict[str, torch.Tensor] = {}
        sr_out = None
        for role in ("anchor", "positive", "negative"):
            clip, sr = self._make_interval_clip(triplet[role]["start_id"], triplet[role]["target_id"])
            if sr_out is None:
                sr_out = sr
            elif sr_out != sr:
                raise ValueError("Sample rate mismatch across clips")
            clips[role] = clip
        return clips, sr_out

    # ---------- dataset API ----------
    def __len__(self):
        return self.n_examples

    def __getitem__(self, idx):
        rng = random.Random(self.seed + idx)
        for _ in range(16):
            try:
                if self.balance_eval:
                    interval, instrument = self.condition_schedule[idx % len(self.condition_schedule)]
                    triplet = self._build_triplet_for_condition(interval, instrument, rng)
                else:
                    interval = rng.choice(self.interval_choices)
                    triplet = self._build_triplet(interval, rng)
                clips, sr = self._load_triplet_audio(triplet)
                if sr != self.target_sr:
                    for k, v in clips.items():
                        clips[k] = torchaudio.functional.resample(v.unsqueeze(0), sr, self.target_sr).squeeze(0)
                    sr = self.target_sr
                return clips, sr, triplet
            except Exception:
                continue
        raise RuntimeError("Failed to sample a valid triplet after multiple attempts")

    @staticmethod
    def describe_triplet(triplet: Dict[str, Dict]) -> None:
        for role in ("anchor", "positive", "negative"):
            entry = triplet[role]
            print(
                f"{role.capitalize():<8} | instrument_str {entry['instrument']:<16} | "
                f"{entry['start_pitch']:>3} → {entry['target_pitch']:>3} "
                f"(Δ {entry['interval']:+d}) | ids {entry['start_id']} → {entry['target_id']}"
            )

