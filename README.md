...
The CE-SSL training logic lives in `lightning_scripts/lightning_ssl_matched_speech_in_noise.py`. This Lightning module selects train and validation datasets from the YAML config, so the same module handles matched speech-in-noise training and AudioSet-only training. It defines how waveform views pass through the audio frontend and encoder + projector, how SSL and supervised losses are combined, and how the optimizer/scheduler are constructed. It uses audio frontends from `robustness.audio_functions.audio_transforms`, encoders and projectors from `lightning_scripts/architectures.py`, and SSL losses from `lightning_scripts/losses/`.
