...
#SBATCH --mem=80Gb
#SBATCH --time=12:00:00 # approx 6 if training classifier from scratch. 10 min if just evaling
...
#                                    --task 'word' \
#                                    --train_epochs 6 \
#                                    --no-with_noise --no-eval_only --no-lr_scheduler --no-use_classifier_ckpt --no-time_avg_rep --with_dropout

srun python3 lightning_scripts/eval_jsin_transfer_matched.py --config_path model_configs/resnet18_barlow_equivariant_lmbda_1e-2_lr_2e-1_w_invar_augment_no_avgpool_eq_lmbda_5e-01.yaml \
                                   --gpus $num_gpus --num_workers $SLURM_JOB_CPUS_PER_NODE \
                                   --model_ckpt_dir model_checkpoints \
                                   --batch_size 192 \
                                   --layer_str 'layer3' \
